/*Solo me tira 3 warnings y no se porque
Al final no hice alta y baja como funciones porque me parecio que modificiaciones es igual a baja con solo una linea diferente :O */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include "Macros_Ranking.h"
#include "macros_constantes.h"
...
int main(int argc, char *argv[]){
	...
	FILE *log;
	FILE *arrangefile;
	FILE *tmpfile=fopen("temporal", w+b);
	int aux_cant_lineas;
	int cant_lineas;
                            ...
                            while(!feof(database)){
                                ...
                                if((line_arrangefile->id) > (line_database->id)){
                                    ...
                                    if((line_arrangefile->id) <= (line_database->id)){   /*Comparo nuevamente, si resulta que el ahora id de arrangefile no supera al de database, entonces es momento de poner el registro da arrangefile en tmpfile*/
                                        ...
                                        fwrite(line_arrangefile, sizeof(book_t),1,tmpfile);

                                        if((line_arrangefile->id) == (line_database->id)) fprintf(log, "%s,%s\n", MSJ_ERROR, MSJ_ERROR_ALTA);

                                        if(aux_cant_lineas==cant_lineas){

                                            fwrite(standby_line, sizeof(book_t),1,tmpfile); /*EScribo la linea que guarde y dps guardo el resto*/
                                        ...
                                        fwrite(standby_line, sizeof(book_t),1,tmpfile);	 /*si resulto que todavia no supera el id, escribimos el registro de*/
                                    }
		...
		case 'M':
				...
				while(!feof(database)){
					...
					if(line_database->id==line_arrangefile->id){ /*si se ecuentra el mismo id, escribimos en el temporal la linea de arrangefile que contiene las modificaciones */
						...
						fwrite(line_arrangefile, sizeof(book_t),1,tmpfile); /*SOLO SE AGREGA ESTO A DIFERENCIA DE BAJA*/
