			...
			switch (stat_programa){
                    ...
                    case ST_ERROR:
								...
								if (cant_intentos!=MAX_CANT_INTENTOS){
									...
									stat_programa=st_anterior;
								}
								else
								{
									fprintf(stderr, "%s", MSJ_ERROR_FATAL);
									return EXIT_FAILURE;
								}
...
int leer_int (void) {    /* la volvi a dejar como estaba con mis modificaciones de ayer*/
	...
	while(j==FALSE || cantidad_intentos!=MAX_CANT_INTENTOS);
	/*if cantidad_intentos==MAX_CANT_INTENTOS){return EXIT_FAILURE;}
	 else return dato;*/
	return dato;
}

t_stat leer_double (t_err *error, double *dato) {
